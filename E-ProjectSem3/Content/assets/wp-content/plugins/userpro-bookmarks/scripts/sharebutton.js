...
a2a_config.vars = {
    vars: ["menu_type", "static_server", "linkname", "linkurl", "linkname_escape", ["ssl", (document.location.protocol != "http:") ? "https://static.addtoany.com/menu" : false], "show_title", "onclick", "num_services", "hide_embeds", "prioritize", "exclude_services", "custom_services", ["templates", {}], "orientation", ["track_links", false],
...
if (a2a_config.ssl && a2a_config.date.getFullYear() == 2015 && a2a_config.date.getMonth() == 11) {
...
var a2a = a2a || {
    ...
    button: function(d) {
        ...
        if (!c.getAttribute("aria-label")) {
            c.setAttribute("aria-label", (f == "feed" ? a2a.c.localize.Subscribe : a2a.c.localize.Share))
        }
        if (c.tagName.toLowerCase() == "a" && a2a.type == "page") {
            c.href = "https://www.addtoany.com/share#url=" + encodeURIComponent(d.linkurl) + "&title=" + encodeURIComponent(d.linkname).replace(/'/g, "%27") + "&description=" + encodeURIComponent(a2a.selection()).replace(/'/g, "%27")
        }
    },
    kit: function(r, k) {
        var ae = a2a.type,
            ...
            u = ["facebook_like", "twitter_tweet", "google_plusone", "google_plus_share", "pinterest_pin"],
        ...
        a2a.kit.facebook_like = function() {
            ...
            var af = function() {
                    ...
                    FB.Event.subscribe("edge.create", function(ag, ah) {
                        a2a.GA.track("Facebook Like", "facebook_like", ag, "pages", "AddToAny Share/Save Button");
                        H("Facebook Like", d)
                    })
                },
                i = a2a.i18n();
                    ...
                    if (!a2a.twitter_bind) {
                        ah.events.bind("click", function(ai) {
                            if (ai && ai.region == "tweet") {
                                ...
                                if (aj && aj.url) {
                                    a2a.GA.track("Twitter Tweet", "twitter_tweet", unescape(aj.url), "pages", "AddToAny Share/Save Button");
                                    H("Twitter Tweet", d)
                                }
                            }
                        });
        ...
        a2a.kit.pinterest_pin = function() {
            ...
            i.href = "//www.pinterest.com/pin/create/button/?url=" + J.url + ((J.media) ? "&media=" + encodeURIComponent(J.media) : "") + ((J.description) ? "&description=" + encodeURIComponent(J.description).replace(/'/g, "%27") : "");
            a2a.add_event(d, "click", function() {
                a2a.GA.track("Pinterest Pin", "pinterest_pin", t, "pages", "AddToAny Share/Save Button");
                H("Pinterest Pin", d)
            });
            d.appendChild(i);
        ...
        a2a.kit.google_plus = function() {
            ...
            d.innerHTML = '<div class="g-plus' + ((J.action == "share") ? "" : "one") + '" data-callback="google_plus_cb_a2a"' + Q + "></div>";
        ...
        for (var Y = 0; Y < z; Y++) {
            ...
            if (!c) {
                ...
                if (!ac) {
                    a2a.add_event(d, "click", (function(i, af, ag, n, ah) {
                        return function(ak) {
                            var al = screen.height,
                                am = 550,
                                ai = 450,
                                aj = "event=service_click&url=" + s(location.href) + "&title=" + s(document.title || "") + "&ev_service=" + s(i) + "&ev_service_type=kit&ev_menu_type=" + ae + "&ev_url=" + s(ag) + "&ev_title=" + s(n).replace(/'/g, "%27");
                            H(af, ah, ak);
                            if (ah.a2a.popup && !a2a.defaultPrevented(ak)) {
                                a2a.preventDefault(ak);
                                window.open(ah.href, "_blank", "toolbar=0,personalbar=0,resizable,scrollbars,status,width=550,height=450,top=" + ((al > ai) ? Math.round(al / 2 - ai / 2) : 40) + ",left=" + Math.round(screen.width / 2 - am / 2))
                            }
    ...
    counters: {
        get: function(b, a, g) {
            ...
            if (h.queued.length == 1) {
                e = f[0] + g + (f[1] || "&callback") + "=a2a.counters." + b + ".cb" + c.n;
    ...
    toggle_dropdown: function(a, e) {
        ...
        if (a == "none") {
            if (!window.addEventListener) {
                ...
                document.removeEventListener(h, a2a[e].doc_mouseup_toggle_dropdown, false);
                a2a.touch_used = null
            }
    ...
    util_frame_make: function(f) {
        ...
        h.src = ((a2a.c.ssl) ? a2a.c.ssl : "http://static.addtoany.com/menu") + "/sm13.html#type=" + f + "&event=load&url=" + b + "&referrer=" + c;
        d.style.top = "0";
    ...
    if (!a2a[a2a.type] && !window["a2a" + a2a.type + "_init"]) {
        ...
        a2a["create_" + a2a.type + "_dropdown"] = function(O, u) {
            var o = a2a.gEl,
                ...
                N = a2a.c,
                A = a2a.ieo(),
                y, p = document.createElement("i"),
                m, l, M, F = document.createElement("a"),
                v = N.icon_color,
            ...
            if (s != "feed") {
                var j = t(K.AnyEmail, "email_form", "email", null, "http://www.addtoany.com/email?linkurl=A2A_LINKURL_ENC&linkname=A2A_LINKNAME_ENC");
                j.className = "a2a_i a2a_emailer";
                j.id = "a2a" + s + "_any_email";
                o(q + "_2_col1").appendChild(j);
                var L = t("Email (mailto)", "email_form", "email", null, "mailto:?subject=A2A_LINKNAME_ENC&body=A2A_LINKURL_ENC");
